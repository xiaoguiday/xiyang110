...
func handleClient(conn net.Conn, isTLS bool) {
	...
	// ----> 修改点 2: 清除握手超时 <----
	_ = conn.SetReadDeadline(time.Time{}) // time.Time{} 表示永不超时
...
func handleAdminPost(w http.ResponseWriter, r *http.Request) {
	...
	case "/device/add":
		...
		cfg.DeviceIDs[did] = DeviceInfo{Expiry: exp, LimitGB: limit, UsedBytes: currentUsage}
		cfg.lock.Unlock()
		if err := cfg.SafeSave(); err != nil {
	...
	case "/account/update":
		...
		if cfg.Accounts[ou] == op {
			...
			if err := cfg.SafeSave(); err != nil {
				...
				w.Header().Set("WWW-Authenticate", `Basic realm="Restricted"`)
				http.Error(w, "Password changed. Please log in again.", http.StatusUnauthorized)
			}
		} else {
			cfg.lock.Unlock()
