...
type Config struct { Settings Settings `json:"settings"`; Accounts map[string]string `json:"accounts"`; DeviceIDs map[string]DeviceInfo `json:"device_ids"`; lock sync.RWMutex }
var globalConfig *Config
var once sync.Once
func GetConfig() *Config { once.Do(func() { globalConfig = &Config{}; if err := globalConfig.load(); err != nil { Print("[!] FATAL: Could not load or create config file: %v", err); os.Exit(1) } }); return globalConfig }
func (c *Config) load() error { c.lock.Lock(); defer c.lock.Unlock(); c.Settings = Settings{ HTTPPort: 80, TLSPort: 443, StatusPort: 9090, DefaultTargetHost: "127.0.0.1", DefaultTargetPort: 22, BufferSize: 8192, Timeout: 300, CertFile: "/etc/stunnel/certs/stunnel.pem", KeyFile: "/etc/stunnel/certs/stunnel.key", UAKeywordWS: "26.4.0", UAKeywordProbe: "1.0", AllowSimultaneousConnections: false, DefaultExpiryDays: 30, DefaultLimitGB: 100, EnableDeviceIDAuth: true, }; c.Accounts = map[string]string{"admin": "admin"}; c.DeviceIDs = make(map[string]DeviceInfo); data, err := ioutil.ReadFile(ConfigFile); if err != nil { if os.IsNotExist(err) { Print("[*] %s not found, creating with default structure.", ConfigFile); return c.save() }; return fmt.Errorf("failed to read config file: %w", err) }; if err := json.Unmarshal(data, c); err != nil { return fmt.Errorf("could not decode config file: %w. Please check its format", err) }; return nil }
func (c *Config) save() error { data, err := json.MarshalIndent(c, "", "  "); if err != nil { return fmt.Errorf("failed to marshal config: %w", err) }; return ioutil.WriteFile(ConfigFile, data, 0644) }
...
func (c *Config) GetAccounts() map[string]string { c.lock.RLock(); defer c.lock.RUnlock(); accts := make(map[string]string); for k, v := range c.Accounts { accts[k] = v }; return accts }
func (c *Config) GetDeviceIDs() map[string]DeviceInfo { c.lock.RLock(); defer c.lock.RUnlock(); devices := make(map[string]DeviceInfo); for k, v := range c.DeviceIDs { if v.MaxSessions == 0 { v.MaxSessions = 1 }; devices[k] = v }; return devices }
...
// ... Metrics, pipeTraffic等模块与上个版本相同，保持高性能实现 ...
...
func handleClient(conn net.Conn, isTLS bool) { defer conn.Close(); cfg := GetConfig(); settings := cfg.GetSettings(); remoteIP, _, _ := net.SplitHostPort(conn.RemoteAddr().String()); connKey := fmt.Sprintf("%s-%d", remoteIP, time.Now().UnixNano()); if tcpConn, ok := conn.(*net.TCPConn); ok { tcpConn.SetKeepAlive(true); tcpConn.SetKeepAlivePeriod(30 * time.Second) }; if settings.EnableIPBlacklist && isIPInList(remoteIP, settings.IPBlacklist) { Print("[-] Connection from blacklisted IP %s rejected.", remoteIP); return }; if settings.EnableIPWhitelist && !isIPInList(remoteIP, settings.IPWhitelist) { Print("[-] Connection from non-whitelisted IP %s rejected.", remoteIP); return }; tlsStr := ""; if isTLS { tlsStr = " (TLS)" }; Print("[+] Connection from %s%s", remoteIP, tlsStr); AddActiveConn(connKey, &ActiveConnInfo{ Writer: conn, IP: remoteIP, ConnKey: connKey, FirstConnection: time.Now(), LastActive: time.Now(), Status: "握手" }); defer RemoveActiveConn(connKey); reader := bufio.NewReader(conn); forwardingStarted := false; var initialData []byte; var headersText string; var finalDeviceID string; var deviceInfo DeviceInfo; for !forwardingStarted { _ = conn.SetReadDeadline(time.Now().Add(time.Duration(settings.Timeout) * time.Second)); req, err := http.ReadRequest(reader); if err != nil { if err != io.EOF { Print("[-] Handshake read error from %s: %v", remoteIP, err) } else { Print("[-] Client %s closed connection during handshake.", remoteIP) }; return }; var headerBuilder strings.Builder; _ = req.Header.Write(&headerBuilder); headersText = req.Method + " " + req.RequestURI + " " + req.Proto + "\r\n" + headerBuilder.String(); body, _ := ioutil.ReadAll(req.Body); initialData = body; clientWSKey := req.Header.Get("Sec-WebSocket-Key"); var found bool; if clientWSKey != "" { finalDeviceID, deviceInfo, found = cfg.FindDeviceByWSKey(clientWSKey) }; if settings.EnableDeviceIDAuth { if !found { Print("[!] Auth Enabled: Invalid or missing Sec-WebSocket-Key from %s. Rejecting.", remoteIP); _, _ = conn.Write([]byte("HTTP/1.1 401 Unauthorized\r\n\r\n")); return }; expiry, err := time.Parse("2006-01-02", deviceInfo.Expiry); if err != nil || time.Now().After(expiry.Add(24*time.Hour)) { Print("[!] Auth Enabled: Device ID %s has expired. Rejecting.", finalDeviceID); _, _ = conn.Write([]byte("HTTP/1.1 403 Forbidden\r\n\r\n")); return }; var deviceUsagePtr *int64; if val, ok := deviceUsage.Load(finalDeviceID); ok { deviceUsagePtr = val.(*int64) } else { newUsage := deviceInfo.UsedBytes; deviceUsage.Store(finalDeviceID, &newUsage); deviceUsagePtr = &newUsage }; if deviceInfo.LimitGB > 0 && atomic.LoadInt64(deviceUsagePtr) >= int64(deviceInfo.LimitGB)*1024*1024*1024 { Print("[!] Auth Enabled: Traffic limit reached for %s. Rejecting.", finalDeviceID); _, _ = conn.Write([]byte("HTTP/1.1 403 Forbidden\r\n\r\n")); return } } else { if !found { finalDeviceID = remoteIP } }; if connInfo, ok := GetActiveConn(connKey); ok { connInfo.DeviceID = finalDeviceID }; ua := req.UserAgent(); if settings.UAKeywordProbe != "" && strings.Contains(ua, settings.UAKeywordProbe) { Print("[*] Received probe from %s for device '%s'. Awaiting WS handshake.", remoteIP, finalDeviceID); _, _ = conn.Write([]byte("HTTP/1.1 200 OK\r\n\r\nOK")); continue }; if settings.UAKeywordWS != "" && strings.Contains(ua, settings.UAKeywordWS) { Print("[*] Received WebSocket handshake from %s for device '%s'.", remoteIP, finalDeviceID); if settings.EnableDeviceIDAuth && found && deviceInfo.MaxSessions > 0 { currentSessionCount := 0; activeConns.Range(func(key, value interface{}) bool { c := value.(*ActiveConnInfo); if c.DeviceID == finalDeviceID && c.Status == "活跃" { currentSessionCount++ }; return true }); if currentSessionCount >= deviceInfo.MaxSessions { Print("[!] Max sessions reached for device %s (%d/%d). Rejecting new connection.", finalDeviceID, currentSessionCount, deviceInfo.MaxSessions); _, _ = conn.Write([]byte("HTTP/1.1 403 Forbidden\r\n\r\n")); return } }; _, _ = conn.Write([]byte("HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\n\r\n")); if connInfo, ok := GetActiveConn(connKey); ok { connInfo.Status = "活跃" }; forwardingStarted = true } else { Print("[!] Unrecognized User-Agent from %s: '%s'. Rejecting.", remoteIP, ua); _, _ = conn.Write([]byte("HTTP/1.1 403 Forbidden\r\n\r\n")); return } }; _ = conn.SetReadDeadline(time.Time{}); targetHost := settings.DefaultTargetHost; targetPort := settings.DefaultTargetPort; if realHost := extractHeaderValue(headersText, "x-real-host"); realHost != "" { if host, portStr, err := net.SplitHostPort(realHost); err == nil { targetHost = host; if p, err := strconv.Atoi(portStr); err == nil { targetPort = p } } else { targetHost = realHost } }; targetAddr := fmt.Sprintf("%s:%d", targetHost, targetPort); Print("[*] Tunneling %s -> %s for device %s", remoteIP, targetAddr, finalDeviceID); targetConn, err := net.DialTimeout("tcp", targetAddr, 10*time.Second); if err != nil { Print("[!] Failed to connect to target %s: %v", targetAddr, err); return }; defer targetConn.Close(); if tcpTargetConn, ok := targetConn.(*net.TCPConn); ok { tcpTargetConn.SetKeepAlive(true); tcpTargetConn.SetKeepAlivePeriod(30 * time.Second) }; if len(initialData) > 0 { _, _ = targetConn.Write(initialData) }; ctx, cancel := context.WithCancel(context.Background()); var wg sync.WaitGroup; wg.Add(2); go pipeTraffic(ctx, &wg, targetConn, conn, connKey, finalDeviceID, true); go pipeTraffic(ctx, &wg, conn, targetConn, connKey, finalDeviceID, false); wg.Wait(); cancel(); Print("[-] Closed connection for %s (Device: %s)", remoteIP, finalDeviceID) }
type copyTracker struct { io.Writer; ConnInfo *ActiveConnInfo; DeviceID string; IsUpload bool; DeviceUsagePtr *int64 }
func (c *copyTracker) Write(p []byte) (n int, err error) { n, err = c.Writer.Write(p); if n > 0 { if c.IsUpload { atomic.AddInt64(&globalBytesSent, int64(n)); atomic.AddInt64(&c.ConnInfo.BytesSent, int64(n)) } else { atomic.AddInt64(&globalBytesReceived, int64(n)); atomic.AddInt64(&c.ConnInfo.BytesReceived, int64(n)) }; if c.DeviceID != "" && c.DeviceID != "unknown_device" && c.DeviceUsagePtr != nil { atomic.AddInt64(c.DeviceUsagePtr, int64(n)) }; c.ConnInfo.LastActive = time.Now() }; return }
func pipeTraffic(ctx context.Context, wg *sync.WaitGroup, dst, src net.Conn, connKey, deviceID string, isUpload bool) { defer wg.Done(); connInfo, ok := GetActiveConn(connKey); if !ok { return }; var deviceUsagePtr *int64; if deviceID != "" && deviceID != "unknown_device" { if val, ok := deviceUsage.Load(deviceID); ok { deviceUsagePtr = val.(*int64) } }; tracker := &copyTracker{ Writer: dst, ConnInfo: connInfo, DeviceID: deviceID, IsUpload: isUpload, DeviceUsagePtr: deviceUsagePtr, }; buf := make([]byte, 32*1024); io.CopyBuffer(tracker, src, buf); if tcpConn, ok := dst.(*net.TCPConn); ok { _ = tcpConn.CloseWrite() } }
...
// --- [Session认证 1] 定义Session和Session存储 ---
...
// --- [Session认证 2] 新的Auth中间件 ---
func authMiddleware(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		cookie, err := r.Cookie(sessionCookieName)
		if err != nil {
			// 如果没有cookie，返回401，前端会知道需要登录
...
func loginHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(storedPass) >= 60 && strings.HasPrefix(storedPass, "$2a$") {
		valid = checkPasswordHash(pass, storedPass) // 昂贵的bcrypt只在这里执行一次
	} else {
	...
	expiry := time.Now().Add(12 * time.Hour) // Session有效期12小时
	...
	http.SetCookie(w, &http.Cookie{
		...
		Expires:  expiry,
		Path:     "/",
		HttpOnly: true, // 防止JS脚本读取cookie
	})
	
	w.WriteHeader(http.StatusOK)
}

// --- [Session认证 4] 新的登出Handler ---
...
func handleAdminPage(w http.ResponseWriter, r *http.Request) {
    // 这个函数现在只处理一个特殊情况: 首次加载页面时，如果没有有效cookie，则提示需要登录
	_, err := r.Cookie(sessionCookieName)
	if err != nil {
		// 模拟Basic Auth的响应，触发浏览器弹出登录框
		w.Header().Set("WWW-Authenticate", `Basic realm="Restricted"`)
		http.Error(w, "Unauthorized.", http.StatusUnauthorized)
		return
	}
	// 如果有有效cookie, 直接提供html文件
...
func main() {
	...
	if err != nil { Print("[!] FATAL: admin.html not found in the current directory: %v", err); os.Exit(1) }
	...
	// --- [Session认证 5] 重构管理后台的路由 ---
	adminMux := http.NewServeMux()
	// 根路径'/'现在需要特殊处理，以便在未登录时触发Basic Auth登录框
	adminMux.HandleFunc("/", handleAdminPage)
	// 新的登录端点，使用Basic Auth进行一次性认证
	adminMux.HandleFunc("/login", loginHandler)
	// 登出端点
	adminMux.HandleFunc("/logout", logoutHandler)

	// 所有API和其他POST请求，都使用新的、高效的authMiddleware进行cookie认证
	adminMux.HandleFunc("/api/", authMiddleware(handleAPI))
	adminMux.HandleFunc("/device/", authMiddleware(handleAdminPost))
	adminMux.HandleFunc("/account/", authMiddleware(handleAdminPost))
	adminMux.HandleFunc("/settings/", authMiddleware(handleAdminPost))
