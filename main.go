...
func (c *Config) load() error {
	...
	if err != nil { if os.IsNotExist(err) { Print("[*] %s not found, creating with default structure.", ConfigFile); return c.save() }; return fmt.Errorf("failed to read config file: %w", err) }
	if err := json.Unmarshal(data, c); err != nil { return fmt.Errorf("could not decode config file: %w. Please check its format", err) }
	return nil
}
...
func (c *Config) GetDeviceIDs() map[string]DeviceInfo {
	...
	for k, v := range c.DeviceIDs { if v.MaxSessions == 0 { v.MaxSessions = 1 }; devices[k] = v }
	return devices
}
...
func UpdateConnTraffic(key string, sent, received int64, deviceID string) { globalBytesSent += sent; globalBytesReceived += received; if conn, ok := GetActiveConn(key); ok { conn.BytesSent += sent; conn.BytesReceived += received; conn.LastActive = time.Now() }; if deviceID != "" && deviceID != "unknown_device" { if val, ok := deviceUsage.Load(deviceID); ok { deviceUsage.Store(deviceID, val.(int64)+sent+received) } else { deviceUsage.Store(deviceID, sent+received) } } }
...
func saveDeviceUsage() { cfg := GetConfig(); cfg.lock.Lock(); defer cfg.lock.Unlock(); isDirty := false; deviceUsage.Range(func(key, value interface{}) bool { id := key.(string); usage := value.(int64); if info, ok := cfg.DeviceIDs[id]; ok { if info.UsedBytes != usage { info.UsedBytes = usage; cfg.DeviceIDs[id] = info; isDirty = true } }; return true }); if isDirty { if err := cfg.save(); err != nil { Print("[!] Failed to save device usage: %v", err) } } }
func collectSystemStatus() { systemStatusMutex.Lock(); defer systemStatusMutex.Unlock(); systemStatus.Uptime = time.Since(startTime).Round(time.Second).String(); cp, err := cpu.Percent(0, false); if err == nil && len(cp) > 0 { systemStatus.CPUPercent, _ = strconv.ParseFloat(fmt.Sprintf("%.1f", cp[0]), 64) }; if cores, err := cpu.Counts(true); err == nil { systemStatus.CPUCores = cores }; if vm, err := mem.VirtualMemory(); err == nil { systemStatus.MemTotal = vm.Total; systemStatus.MemUsed = vm.Used; systemStatus.MemPercent, _ = strconv.ParseFloat(fmt.Sprintf("%.1f", vm.UsedPercent), 64) }; systemStatus.BytesSent = globalBytesSent; systemStatus.BytesReceived = globalBytesReceived }
