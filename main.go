...
// --- [MERGE POINT 2] Upgraded DeviceInfo struct ---
...
// --- [MERGE POINT 3] Upgraded GetDeviceIDs for backward compatibility ---
func (c *Config) GetDeviceIDs() map[string]DeviceInfo {
	c.lock.RLock()
	defer c.lock.RUnlock()
	devices := make(map[string]DeviceInfo)
	for k, v := range c.DeviceIDs {
		// Automatically provide default values for new fields if they don't exist
		if v.SecWSKey == "" {
			v.SecWSKey = "" // Default to empty string
		}
		if v.MaxSessions == 0 {
			v.MaxSessions = 1 // Default to 1 session
		}
		devices[k] = v
	}
	return devices
...
// --- [MERGE POINT 4] New password hashing functions ---
...
// ---------------------- METRICS (监控模块) ------------------------
// =================================================================
// (This entire section is unchanged from the original file)
...
var (
	globalBytesSent     int64; globalBytesReceived int64; activeConns sync.Map; deviceUsage sync.Map; startTime = time.Now(); systemStatus SystemStatus; systemStatusMutex sync.RWMutex; adminPanelHTML []byte
)
func InitMetrics() {
	cfg := GetConfig()
	devices := cfg.GetDeviceIDs()
	for id, info := range devices { deviceUsage.Store(id, info.UsedBytes) }
}
...
func runPeriodicTasks() {
	...
	go func() { for range saveTicker.C { saveDeviceUsage() } }()
	statusTicker := time.NewTicker(2 * time.Second)
	go func() { for range statusTicker.C { collectSystemStatus() } }()
}
func saveDeviceUsage() {
	...
	deviceUsage.Range(func(key, value interface{}) bool {
		id := key.(string); usage := value.(int64)
		if info, ok := cfg.DeviceIDs[id]; ok {
			if info.UsedBytes != usage {
				info.UsedBytes = usage; cfg.DeviceIDs[id] = info; isDirty = true
			}
		}
		return true
	})
	if isDirty { if err := cfg.save(); err != nil { Print("[!] Failed to save device usage: %v", err) } }
}
...
func handleClient(conn net.Conn, isTLS bool) {
	defer conn.Close()
	cfg := GetConfig(); settings := cfg.GetSettings()
	...
	forwardingStarted := false; var initialData []byte; var headersText string
	for !forwardingStarted {
		...
		var headerBuilder strings.Builder; _ = req.Header.Write(&headerBuilder)
		headersText = req.Method + " " + req.RequestURI + " " + req.Proto + "\r\n" + headerBuilder.String()
		body, _ := ioutil.ReadAll(req.Body); initialData = body
		deviceID := req.Header.Get("x-device-id"); if deviceID == "" { deviceID = req.Header.Get("device-id") }
		finalDeviceID := deviceID; if finalDeviceID == "" { finalDeviceID = "unknown_device" }
		if connInfo, ok := GetActiveConn(connKey); ok { connInfo.DeviceID = finalDeviceID }
		if settings.EnableDeviceIDAuth {
			if deviceID == "" { Print("[!] Auth Enabled: No device ID from %s. Rejecting.", remoteIP); _, _ = conn.Write([]byte("HTTP/1.1 403 Forbidden\r\n\r\n")); return }
			deviceInfo, ok := cfg.GetDeviceIDs()[deviceID]
			if !ok { Print("[!] Auth Enabled: Invalid device ID '%s' from %s. Rejecting.", deviceID, remoteIP); _, _ = conn.Write([]byte("HTTP/1.1 403 Forbidden\r\n\r\n")); return }
			expiry, err := time.Parse("2006-01-02", deviceInfo.Expiry)
			if err != nil || time.Now().After(expiry.Add(24*time.Hour)) { Print("[!] Auth Enabled: Device ID %s has expired. Rejecting.", deviceID); _, _ = conn.Write([]byte("HTTP/1.1 403 Forbidden\r\n\r\n")); return }
			usage, _ := deviceUsage.LoadOrStore(deviceID, int64(0))
			if deviceInfo.LimitGB > 0 && usage.(int64) >= int64(deviceInfo.LimitGB)*1024*1024*1024 { Print("[!] Auth Enabled: Traffic limit reached for %s. Rejecting.", deviceID); _, _ = conn.Write([]byte("HTTP/1.1 403 Forbidden\r\n\r\n")); return }
		}
		ua := req.UserAgent()
		if settings.UAKeywordProbe != "" && strings.Contains(ua, settings.UAKeywordProbe) { Print("[*] Received probe from %s for device '%s'. Awaiting WS handshake.", remoteIP, finalDeviceID); _, _ = conn.Write([]byte("HTTP/1.1 200 OK\r\n\r\nOK")); continue }
		if settings.UAKeywordWS != "" && strings.Contains(ua, settings.UAKeywordWS) {
			...
			if !settings.AllowSimultaneousConnections && finalDeviceID != "unknown_device" {
				...
				activeConns.Range(func(key, value interface{}) bool {
					...
					if c.DeviceID == finalDeviceID && c.ConnKey != connKey { existingConnKey = c.ConnKey; return false }
					return true
				})
				if existingConnKey != "" { Print("[!] WS Check: Simultaneous connection rejected for device %s. Blocked by: %s", finalDeviceID, existingConnKey); _, _ = conn.Write([]byte("HTTP/1.1 403 Forbidden\r\n\r\n")); return }
			}
			_, _ = conn.Write([]byte("HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\n\r\n"))
			if connInfo, ok := GetActiveConn(connKey); ok { connInfo.Status = "活跃" }
			forwardingStarted = true
		} else { Print("[!] Unrecognized User-Agent from %s: '%s'. Rejecting.", remoteIP, ua); _, _ = conn.Write([]byte("HTTP/1.1 403 Forbidden\r\n\r\n")); return }
	}
	...
	if realHost := extractHeaderValue(headersText, "x-real-host"); realHost != "" {
		...
		} else { targetHost = realHost }
	}
	targetAddr := fmt.Sprintf("%s:%d", targetHost, targetPort); Print("[*] Tunneling %s -> %s", remoteIP, targetAddr)
	targetConn, err := net.DialTimeout("tcp", targetAddr, 10*time.Second)
	if err != nil { Print("[!] Failed to connect to target %s: %v", targetAddr, err); return }
	...
	wg.Wait(); cancel(); Print("[-] Closed connection for %s", remoteIP)
}
	...
	for {
		...
		default:
			...
			if n > 0 {
				...
				connInfo, ok := GetActiveConn(connKey); if !ok { return }
				if isUpload { UpdateConnTraffic(connKey, int64(n), 0, connInfo.DeviceID) } else { UpdateConnTraffic(connKey, 0, int64(n), connInfo.DeviceID) }
			}
			if err != nil { if tcpConn, ok := dst.(*net.TCPConn); ok { _ = tcpConn.CloseWrite() }; return }
		}
	}
}
...
func runCommand(command string, args ...string) (bool, string) {
	cmd := exec.Command(command, args...); var out, stderr bytes.Buffer; cmd.Stdout = &out; cmd.Stderr = &stderr; err := cmd.Run()
	if err != nil { return false, fmt.Sprintf("Command '%s %s' failed: %v, Stderr: %s", command, strings.Join(args, " "), err, stderr.String()) }
	return true, out.String()
}
func manageSshUser(username, password, action string) (bool, string) {
	if os.Geteuid() != 0 { return false, "此操作需要 root 权限。" }
	if !regexp.MustCompile(`^[a-z_][a-z0-9_-]{0,30}$`).MatchString(username) { return false, "无效的用户名。请使用小写字母、数字、下划线或连字符。" }
	sshdConfigPath := "/etc/ssh/sshd_config"; startMarker := fmt.Sprintf("# WSSUSER_BLOCK_START_%s", username); endMarker := fmt.Sprintf("# WSSUSER_BLOCK_END_%s", username)
	cleanSshdConfig := func() error {
		...
		lines := strings.Split(string(content), "\n"); var newLines []string; inBlock := false
		for _, line := range lines {
			if strings.Contains(line, startMarker) { inBlock = true; continue }
			if strings.Contains(line, endMarker) { inBlock = false; continue }
			if !inBlock { newLines = append(newLines, line) }
		}
		return ioutil.WriteFile(sshdConfigPath, []byte(strings.Join(newLines, "\n")), 0644)
	}
	restartSshd := func() (bool, string) { success, msg := runCommand("systemctl", "restart", "sshd"); if !success { success, msg = runCommand("systemctl", "restart", "ssh") }; return success, msg }
	if action == "delete" {
		if err := cleanSshdConfig(); err != nil { return false, fmt.Sprintf("清理 sshd_config 失败: %v", err) }
		cmd := exec.Command("id", username)
		if cmd.Run() == nil {
			delSuccess, msg := runCommand("userdel", "-r", username)
			if !delSuccess { return false, fmt.Sprintf("删除用户失败: %s", msg) }
		}
		sshdRestartSuccess, msg := restartSshd(); if !sshdRestartSuccess { return false, fmt.Sprintf("SSHD/SSH 重启失败: %s", msg) }
		return true, fmt.Sprintf("用户 %s 及相关配置已成功删除。", username)
	}
	if action == "create" {
		if password == "" { return false, "创建用户时必须提供密码。" }
		if err := cleanSshdConfig(); err != nil { return false, fmt.Sprintf("清理旧 sshd_config 失败: %v", err) }
		cmd := exec.Command("id", username)
		if cmd.Run() != nil {
			addSuccess, msg := runCommand("useradd", "-m", "-s", "/bin/bash", username)
			if !addSuccess { return false, fmt.Sprintf("创建用户失败: %s", msg) }
		}
		chpasswdCmd := exec.Command("chpasswd"); chpasswdCmd.Stdin = strings.NewReader(fmt.Sprintf("%s:%s", username, password))
		if err := chpasswdCmd.Run(); err != nil { return false, fmt.Sprintf("设置密码失败: %v", err) }
		f, err := os.OpenFile(sshdConfigPath, os.O_APPEND|os.O_WRONLY, 0644)
		if err != nil { return false, fmt.Sprintf("打开 sshd_config 失败: %v", err) }
		newBlock := fmt.Sprintf("\n%s\nMatch User %s Address 127.0.0.1,::1\n    PasswordAuthentication yes\n    PermitTTY yes\n    AllowTcpForwarding yes\nMatch User %s Address *,!127.0.0.1,!::1\n    PasswordAuthentication no\n    PermitTTY no\n    AllowTcpForwarding no\n%s\n", startMarker, username, username, endMarker)
		_, _ = f.WriteString(newBlock); f.Close()
		sshdRestartSuccess, msg := restartSshd(); if !sshdRestartSuccess { return false, fmt.Sprintf("SSHD/SSH 重启失败: %s", msg) }
...
// --- [MERGE POINT 5] Upgraded basicAuth to handle hashed passwords ---
func basicAuth(handler http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Check for hash or plain text
		valid := false
		if len(storedPass) >= 60 && strings.HasPrefix(storedPass, "$2a$") {
			valid = checkPasswordHash(pass, storedPass) // Hash comparison
		} else {
			valid = (pass == storedPass) // Plain text comparison for backward compatibility
		}
...
func formatBytes(b int64) string {
	const unit = 1024; if b < unit { return fmt.Sprintf("%d B", b) }; div, exp := int64(unit), 0
	for n := b / unit; n >= unit; n /= unit { div *= unit; exp++ }; return fmt.Sprintf("%.2f %cB", float64(b)/float64(div), "KMGTPE"[exp])
}
type APIConnectionResponse struct {
	DeviceID string `json:"device_id"`; Status string `json:"status"`; SentStr string `json:"sent_str"`; RcvdStr string `json:"rcvd_str"`; SpeedStr string `json:"speed_str"`; RemainingStr string `json:"remaining_str"`; Expiry string `json:"expiry"`; IP string `json:"ip"`; FirstConn string `json:"first_conn"`; LastActive string `json:"last_active"`; ConnKey string `json:"conn_key"`
}

// handleAPI is mostly unchanged, but it will benefit from the upgraded GetDeviceIDs
func handleAPI(w http.ResponseWriter, r *http.Request) {
	...
	case "/api/connections":
		var conns []*ActiveConnInfo; activeConns.Range(func(key, value interface{}) bool { conns = append(conns, value.(*ActiveConnInfo)); return true })
		sort.Slice(conns, func(i, j int) bool {
			statusOrder := map[string]int{"活跃": 0, "握手": 1}; if statusOrder[conns[i].Status] != statusOrder[conns[j].Status] { return statusOrder[conns[i].Status] < statusOrder[conns[j].Status] }; return conns[i].FirstConnection.Before(conns[j].FirstConnection)
		})
		resp := []APIConnectionResponse{}; now := time.Now()
		for _, c := range conns {
			if c.Status == "活跃" {
				timeDelta := now.Sub(c.LastSpeedUpdateTime).Seconds()
				if timeDelta >= 2 {
					currentTotalBytes := c.BytesSent + c.BytesReceived; bytesDelta := currentTotalBytes - c.LastTotalBytesForSpeed
			...
			resp = append(resp, APIConnectionResponse{
				DeviceID: c.DeviceID, Status: c.Status, SentStr: formatBytes(c.BytesSent), RcvdStr: formatBytes(c.BytesReceived), SpeedStr: fmt.Sprintf("%s/s", formatBytes(int64(c.CurrentSpeedBps))), RemainingStr: remainingStr, Expiry: deviceInfo.Expiry, IP: c.IP, FirstConn: c.FirstConnection.Format("15:04:05"), LastActive: c.LastActive.Format("15:04:05"), ConnKey: c.ConnKey,
			})
		}
		sendJSON(w, http.StatusOK, resp)
	case "/api/kick":
		connKey, _ := reqData["conn_key"].(string)
		if conn, ok := GetActiveConn(connKey); ok { _ = conn.Writer.Close(); RemoveActiveConn(connKey); sendJSON(w, http.StatusOK, map[string]string{"status": "ok", "message": "连接已踢掉"}) } else { sendJSON(w, http.StatusOK, map[string]string{"status": "error", "message": "连接未找到"}) }
	case "/api/clear":
		connKey, _ := reqData["conn_key"].(string); RemoveActiveConn(connKey); sendJSON(w, http.StatusOK, map[string]string{"status": "ok"})
	case "/api/device_usage":
		usageMap := make(map[string]int64); deviceUsage.Range(func(key, value interface{}) bool { usageMap[key.(string)] = value.(int64); return true }); sendJSON(w, http.StatusOK, usageMap)
	case "/api/logs":
		sendJSON(w, http.StatusOK, logBuffer.GetLogs())
	case "/api/server_status":
		systemStatusMutex.RLock(); defer systemStatusMutex.RUnlock(); sendJSON(w, http.StatusOK, systemStatus)
	case "/api/devices":
		sendJSON(w, http.StatusOK, cfg.GetDeviceIDs())
	case "/api/settings":
		sendJSON(w, http.StatusOK, cfg.GetSettings())
	case "/api/settings/toggle_device_auth":
		enable, _ := reqData["enable"].(bool); cfg.lock.Lock(); cfg.Settings.EnableDeviceIDAuth = enable; cfg.lock.Unlock()
		if err := cfg.SafeSave(); err != nil { sendJSON(w, http.StatusInternalServerError, map[string]string{"status": "error", "message": err.Error()}) } else { statusText := "开启"; if !enable { statusText = "关闭" }; sendJSON(w, http.StatusOK, map[string]string{"status": "ok", "message": fmt.Sprintf("Device ID 验证已%s", statusText)}) }
	case "/api/ssh/create", "/api/ssh/delete":
		action := filepath.Base(r.URL.Path); username, _ := reqData["username"].(string); password, _ := reqData["password"].(string)
		success, message := manageSshUser(username, password, action); status := "ok"; if !success { status = "error" }; sendJSON(w, http.StatusOK, map[string]string{"status": status, "message": message})
	default:
		http.NotFound(w, r)
	}
}

// --- [MERGE POINT 6] Upgraded handleAdminPost with new device and account logic ---
func handleAdminPost(w http.ResponseWriter, r *http.Request) {
	...
	switch r.URL.Path {
	case "/device/add": // New logic from upgrade file
		...
		if hasMaxSessions {
			...
			if ms < 0 || ms > 5 {
				ms = 1 // Default to 1 if out of range
			}
		}
	...
	case "/device/delete": // Original logic
		did, _ := reqData["device_id"].(string); cfg.lock.Lock(); delete(cfg.DeviceIDs, did); cfg.lock.Unlock(); deviceUsage.Delete(did)
		if err := cfg.SafeSave(); err != nil { sendJSON(w, http.StatusOK, map[string]string{"status": "error", "message": err.Error()}) } else { sendJSON(w, http.StatusOK, map[string]string{"status": "ok", "message": "删除成功"}) }

	case "/device/reset_traffic": // Original logic
		did, _ := reqData["device_id"].(string); deviceUsage.Store(did, int64(0)); cfg.lock.Lock()
		if info, ok := cfg.DeviceIDs[did]; ok { info.UsedBytes = 0; cfg.DeviceIDs[did] = info }; cfg.lock.Unlock()
		if err := cfg.SafeSave(); err != nil { sendJSON(w, http.StatusOK, map[string]string{"status": "error", "message": err.Error()}) } else { sendJSON(w, http.StatusOK, map[string]string{"status": "ok", "message": "流量已重置"}) }

	case "/account/update": // New logic from upgrade file
		ou, _ := reqData["old_user"].(string); op, _ := reqData["old_pass"].(string)
		nu, _ := reqData["new_user"].(string); np, _ := reqData["new_pass"].(string)
		...
		cfg.lock.Lock()
		// If username is changed, delete the old one
		if ou != nu {
	...
	case "/settings/save": // Original logic
		oldSettings := cfg.GetSettings(); oldPorts := []int{oldSettings.HTTPPort, oldSettings.TLSPort, oldSettings.StatusPort}
		var newSettings Settings; settingsBytes, _ := json.Marshal(reqData); _ = json.Unmarshal(settingsBytes, &newSettings)
		if wl, ok := reqData["ip_whitelist"].(string); ok { newSettings.IPWhitelist = strings.Split(wl, ",") }
		if bl, ok := reqData["ip_blacklist"].(string); ok { newSettings.IPBlacklist = strings.Split(bl, ",") }
		cfg.lock.Lock(); cfg.Settings = newSettings; cfg.lock.Unlock()
		if err := cfg.SafeSave(); err != nil { sendJSON(w, http.StatusOK, map[string]string{"status": "error", "message": fmt.Sprintf("保存失败: %v", err)}); return }
		newPorts := []int{newSettings.HTTPPort, newSettings.TLSPort, newSettings.StatusPort}; portsChanged := false
		for i := range oldPorts { if oldPorts[i] != newPorts[i] { portsChanged = true; break } }
		if portsChanged {
			sendJSON(w, http.StatusOK, map[string]string{"status": "ok", "message": "端口设置已更改, 服务正在重启..."})
			go func() {
				time.Sleep(1 * time.Second); Print("[*] Port settings changed. Restarting server...")
				executable, _ := os.Executable(); cmd := exec.Command(executable, os.Args[1:]...); cmd.Stdout, cmd.Stderr, cmd.Stdin = os.Stdout, os.Stderr, os.Stdin
				if err := cmd.Start(); err != nil { Print("[!] FATAL: Failed to restart process: %v", err); os.Exit(1) }
				os.Exit(0)
			}()
		} else { Print("[*] Settings updated and hot-reloaded successfully."); sendJSON(w, http.StatusOK, map[string]string{"status": "ok", "message": "设置已保存并热加载成功！"}) }
	}
}

func handleAdminPage(w http.ResponseWriter, r *http.Request) {
	if r.URL.Path != "/" { http.NotFound(w, r); return }
	user := r.Context().Value("user").(string); systemStatusMutex.RLock()
	activeCount := 0; activeConns.Range(func(key, value interface{}) bool { if value.(*ActiveConnInfo).Status == "活跃" { activeCount++ }; return true })
	memStr := "N/A"
	if systemStatus.MemTotal > 0 { memStr = fmt.Sprintf("%.1f/%.1f GB (%.1f%%)", float64(systemStatus.MemUsed)/1073741824, float64(systemStatus.MemTotal)/1073741824, systemStatus.MemPercent) }
...
func main() {
	...
	if err != nil { Print("[!] FATAL: admin.html not found in the current directory: %v", err); Print("[!] Please make sure 'admin.html' is in the same folder as the executable."); os.Exit(1) }
	Print("[*] WSTunnel-Go starting...")
	cfg := GetConfig(); InitMetrics(); settings := cfg.GetSettings(); runPeriodicTasks()
	go func() {
		addr := fmt.Sprintf("0.0.0.0:%d", settings.HTTPPort); ln, err := net.Listen("tcp", addr)
		if err != nil { Print("[!] FATAL: Failed to listen on %s: %v", addr, err); os.Exit(1) }
		Print("[*] WS on %s", addr)
		for { conn, err := ln.Accept(); if err != nil { Print("[!] WS accept error: %v", err); continue }; go handleClient(conn, false) }
	}()
	if _, err := os.Stat(settings.CertFile); err == nil {
		if _, err := os.Stat(settings.KeyFile); err == nil {
			go func() {
				...
				if err != nil { Print("[!] Cert warning: %v. WSS server will not start.", err); return }
				ln, err := tls.Listen("tcp", addr, &tls.Config{Certificates: []tls.Certificate{cert}})
				if err != nil { Print("[!] FATAL: Failed to listen on %s (TLS): %v", addr, err); return }
				Print("[*] WSS on %s", addr)
				for { conn, err := ln.Accept(); if err != nil { Print("[!] WSS accept error: %v", err); continue }; go handleClient(conn, true) }
			}()
		}
	...
	adminMux.HandleFunc("/logout", func(w http.ResponseWriter, r *http.Request) { w.Header().Set("WWW-Authenticate", `Basic realm="Restricted"`); http.Error(w, "Unauthorized", http.StatusUnauthorized) })
	adminAddr := fmt.Sprintf("0.0.0.0:%d", settings.StatusPort)
	Print("[*] Status on http://127.0.0.1:%d", settings.StatusPort)
	if err := http.ListenAndServe(adminAddr, adminMux); err != nil { Print("[!] FATAL: Failed to start admin server on %s: %v", adminAddr, err); os.Exit(1) }
}
