...
func handleClient(conn net.Conn, isTLS bool) {
	...
	forwardingStarted := false; var initialData []byte; var headersText string; var finalDeviceID string; var deviceInfo DeviceInfo
	for !forwardingStarted {
		_ = conn.SetReadDeadline(time.Now().Add(time.Duration(settings.Timeout) * time.Second))
		req, err := http.ReadRequest(reader)
		if err != nil { if err != io.EOF { Print("[-] Handshake read error from %s: %v", remoteIP, err) } else { Print("[-] Client %s closed connection during handshake.", remoteIP) }; return }
		var headerBuilder strings.Builder; _ = req.Header.Write(&headerBuilder); headersText = req.Method + " " + req.RequestURI + " " + req.Proto + "\r\n" + headerBuilder.String(); body, _ := ioutil.ReadAll(req.Body); initialData = body
		clientWSKey := req.Header.Get("Sec-WebSocket-Key"); var found bool
		...
		if settings.UAKeywordWS != "" && strings.Contains(ua, settings.UAKeywordWS) {
			...
			if settings.EnableDeviceIDAuth && found && deviceInfo.MaxSessions > 0 {
				currentSessionCount := 0; activeConns.Range(func(key, value interface{}) bool { c := value.(*ActiveConnInfo); if c.DeviceID == finalDeviceID && c.Status == "活跃" { currentSessionCount++ }; return true })
				if currentSessionCount >= deviceInfo.MaxSessions { Print("[!] Max sessions reached for device %s (%d/%d). Rejecting new connection.", finalDeviceID, currentSessionCount, deviceInfo.MaxSessions); _, _ = conn.Write([]byte("HTTP/1.1 403 Forbidden\r\n\r\n")); return }
			}
			_, _ = conn.Write([]byte("HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\n\r\n")); if connInfo, ok := GetActiveConn(connKey); ok { connInfo.Status = "活跃" }; forwardingStarted = true
	...
	if realHost := extractHeaderValue(headersText, "x-real-host"); realHost != "" { if host, portStr, err := net.SplitHostPort(realHost); err == nil { targetHost = host; if p, err := strconv.Atoi(portStr); err == nil { targetPort = p } } else { targetHost = realHost } }
	targetAddr := fmt.Sprintf("%s:%d", targetHost, targetPort); Print("[*] Tunneling %s -> %s for device %s", remoteIP, targetAddr, finalDeviceID)
